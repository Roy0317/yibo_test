...
public class PlayPaneSimpleAdapter extends LAdapter<BallonRules> {
    ...
    public static final String TAG = PlayPaneSimpleAdapter.class.getSimpleName();
    ...
    private boolean isExpanded = true;//玩法栏是否处于展现状态


    public PlayPaneSimpleAdapter(Context mContext, List<BallonRules> mDatas, int layoutId) {
    ...
    public void convert(final int position, LViewHolder holder, ViewGroup parent, final BallonRules item) {
        ...
        final TouzhuSimpleFuncView weishu = holder.getView(R.id.weishu_view);
        final TextView playRule = holder.getView(R.id.play_rule);
        TouzhuSimpleFuncView funcView = holder.getView(R.id.func_view);
...
//        Utils.LOG(TAG, "the figure out column == " + column);
        ...
        ballViews.setNumColumns(7);
        JianjinBallPanSimpleAdapter ballPanAdapter = new JianjinBallPanSimpleAdapter(context, item.getBallonsInfo(), R.layout.jianjin_pan_ball_simple_item);
