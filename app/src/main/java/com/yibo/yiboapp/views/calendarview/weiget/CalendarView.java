package com.yibo.yiboapp.views.calendarview.weiget;
...
import com.yibo.yiboapp.views.calendarview.bean.AttrsBean;
import com.yibo.yiboapp.views.calendarview.bean.DateBean;
import com.yibo.yiboapp.views.calendarview.listener.CalendarViewAdapter;
import com.yibo.yiboapp.views.calendarview.listener.OnMultiChooseListener;
import com.yibo.yiboapp.views.calendarview.listener.OnPagerChangeListener;
import com.yibo.yiboapp.views.calendarview.listener.OnSingleChooseListener;
import com.yibo.yiboapp.views.calendarview.utils.CalendarUtil;
import com.yibo.yiboapp.views.calendarview.utils.SolarUtil;
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < ta.getIndexCount(); i++) {
            ...
            if (attr == R.styleable.CalendarView_show_last_next) {
                ...
                mAttrsBean.setShowLunar(ta.getBoolean(attr, false));
            } else if (attr == R.styleable.CalendarView_show_holiday) {
                mAttrsBean.setShowHoliday(ta.getBoolean(attr, false));
            } else if (attr == R.styleable.CalendarView_show_term) {
                mAttrsBean.setShowTerm(ta.getBoolean(attr, false));
            } else if (attr == R.styleable.CalendarView_switch_choose) {
                mAttrsBean.setSwitchChoose(ta.getBoolean(attr, false));
...
//                mAttrsBean.setDayBg(ta.getResourceId(attr, mAttrsBean.getDayBg()));
    ...
    public void init() {
        ...
        if (mAttrsBean.getChooseType() == 1) {
            ...
            List<int[]> multiDates = mAttrsBean.getMultiDates();
            if (multiDates != null) {
    ...
    private boolean isIllegal(int[] destDate) {
        ...
        if (CalendarUtil.dateToMillis(destDate) > CalendarUtil.dateToMillis(endDate)) {
            //处理12月份特殊情况
            if (destDate[2] <= 31 && endDate[1] == destDate[1] && endDate[0] == destDate[0]) {
                return true;
            }
            return false;
        }
     ...
     * @return
     */
    public CalendarView setDefaultMultiDate(List<String> dates) {
        List<int[]> multiDates = new ArrayList<>();
        for (String date : dates) {
            int[] d = CalendarUtil.strToArray(date);
            if (isIllegal(d)) {
                multiDates.add(d);
                int datePosition = CalendarUtil.dateToPosition(d[0], d[1], startDate[0], startDate[1]);
                positions.add(datePosition);
                setChooseDate(d[2], true, datePosition);
            }
        }
        mAttrsBean.setMultiDates(multiDates);
