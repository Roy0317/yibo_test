...
public class MonthView extends ViewGroup {
    ...
    public void setDateList(List<DateBean> dates, int currentMonthDays) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            ImageView image_circle = null;//当天小红点
            if (item_layout != 0 && calendarViewAdapter != null) {
                ...
                view = LayoutInflater.from(mContext).inflate(R.layout.item_sign_in_calendar, null);
                solarDay = view.findViewById(R.id.solar_day);
                lunarDay = view.findViewById(R.id.lunar_day);
                image_circle = view.findViewById(R.id.image_circle);
            }
            ...
            if (date.getType() == 0 || date.getType() == 2) {
                solarDay.setTextColor(Color.TRANSPARENT);
            }

            //显示当天
            if (image_circle != null) {
                if (date.isToday()) {
                    image_circle.setVisibility(VISIBLE);
                } else {
            ...
            if (mAttrsBean.getChooseType() == 1 && multiDates != null) {
                for (int[] d : multiDates) {
                    if (date.getType() == 1
                            && d[0] == date.getSolar()[0]
                            && d[1] == date.getSolar()[1]
                            && d[2] == date.getSolar()[2]) {
...
//            });
    ...
    private void setDayColor(View v, int type) {
        FrameLayout flLayout = v.findViewById(R.id.flLayout);
//        TextView lunarDay =  v.findViewById(R.id.lunar_day);
//        solarDay.setTextSize(mAttrsBean.getSizeSolar());
//        lunarDay.setTextSize(mAttrsBean.getSizeLunar());
        if (type == 0) {
            flLayout.setBackgroundResource(R.drawable.shape_white_bg5);
//            solarDay.setTextColor(mAttrsBean.getColorSolar());
...
//            }
            ...
            flLayout.setBackgroundResource(R.drawable.shape_yellow_bg5);
...
//            lunarDay.setTextColor(mAttrsBean.getColorChoose());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int itemWidth = widthSpecSize / COLUMN - Utils.dip2px(mContext, 1);
...
//        int dx = (getMeasuredWidth() - itemWidth * COLUMN) / (COLUMN * 2);
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            int left = i % COLUMN * itemWidth + ((2 * (i % COLUMN) + 1));
