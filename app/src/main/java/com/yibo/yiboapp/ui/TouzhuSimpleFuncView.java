 ...
 * 万千百十个位数view
 */
public class TouzhuSimpleFuncView extends LinearLayout {


    public static final String TAG = TouzhuSimpleFuncView.class.getSimpleName();
    Context mContext;
    int viewSize;
    ...
    public TouzhuSimpleFuncView(Context context) {
        super(context);
        mContext = context;
    }

    public TouzhuSimpleFuncView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TouzhuSimpleFuncView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                ballonView.setTextSize(8);
        ...
        for (int i = 0; i < numList.size(); i++) {
            if (viewType == Constant.BALLON_VIEW) {
                ...
                int margin = (int) mContext.getResources().getDimension(R.dimen.ballon_margin_when_touzhu);
