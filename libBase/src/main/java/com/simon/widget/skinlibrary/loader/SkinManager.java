...
import com.simon.utils.FileUtil;
...
import com.simon.widget.skinlibrary.utils.ResourcesCompat;
import com.simon.widget.skinlibrary.utils.TypefaceUtils;
...
public class SkinManager implements ISkinLoader {
    ...
    public void init(Context ctx, boolean cleanFile) {
        context = ctx.getApplicationContext();
        TypefaceUtils.CURRENT_TYPEFACE = TypefaceUtils.getTypeface(context);
        setUpSkinFile(context, cleanFile);
        if (SkinConfig.isInExtendMode(ctx)) {
            SkinManager.getInstance().extendMode();
        } else {
     ...
     * @param cleanFile
     */
    private void setUpSkinFile(Context context, boolean cleanFile) {
        try {
            String[] skinFiles = context.getAssets().list(FileUtil.SKIN_DIR_NAME);
            for (String skinName : skinFiles) {
                File file = new File(FileUtil.getSkinDir(context), skinName);
                if (cleanFile)
                    FileUtil.delete(file);
                //文件不存在进行拷贝
                if (!file.exists()) {
                    FileUtil.CopyAssetsToXXX(context, file,
                            FileUtil.SKIN_DIR_NAME + File.separator + skinName);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void restoreDefaultTheme() {
        ...
        isDefaultSkin = true;
        SkinConfig.setExtendMode(context, false);
    ...
    public boolean isExtendMode() {
        return SkinConfig.isInExtendMode(context);
    }
    ...
    public void loadSkin(String skinName, final SkinLoaderListener callback) {
        ...
        new AsyncTask<String, Void, Resources>() {
            ...
            protected Resources doInBackground(String... params) {
                try {
                    if (params.length == 1) {
                        String skinPkgPath = FileUtil.getSkinDir(context) + File.separator + params[0];
    ...
    public void extendMode() {
        if (!isDefaultSkin) {
            restoreDefaultTheme();
        }
        SkinConfig.setExtendMode(context, true);
        notifySkinUpdate();
    }
    //endregion
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public ColorStateList getExtendColorStateList(int resId) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public int getExtendColor(int resId) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public Drawable getExtendDrawable(String resName) {
